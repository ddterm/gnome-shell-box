locals {
  ...
  # renovate: datasource=custom.html depName=Fedora-Everything-netinst-x86_64 versioning=regex:^(?<major>[0-9]+)_Beta-(?<minor>[0-9]+)\.(?<patch>[0-9]+)$ extractVersion=(^|/)Fedora-Everything-netinst-x86_64-(?<version>[^/]+)\.iso$ registryUrl=https://dl.fedoraproject.org/pub/fedora/linux/releases/test/44_Beta/Everything/x86_64/iso/
  fedora44_version = "44_Beta-1.2"
  # renovate: datasource=custom.html depName=Fedora-Silverblue-ostree-x86_64 versioning=regex:^(?<major>[0-9]+)_Beta-(?<minor>[0-9]+)\.(?<patch>[0-9]+)$ extractVersion=(^|/)Fedora-Silverblue-ostree-x86_64-(?<version>[^/]+)\.iso$ registryUrl=https://dl.fedoraproject.org/pub/fedora/linux/releases/test/44_Beta/Silverblue/x86_64/iso/
  silverblue44_version = "44_Beta-1.2"
}
...
build {
  ...
  source "qemu.fedora" {
    name = "fedora44"
    output_directory = "output-${source.name}"
    iso_url = "https://dl.fedoraproject.org/pub/fedora/linux/releases/test/44_Beta/Everything/x86_64/iso/Fedora-Everything-netinst-x86_64-${local.fedora44_version}.iso"
    iso_checksum = "file:https://dl.fedoraproject.org/pub/fedora/linux/releases/test/44_Beta/Everything/x86_64/iso/Fedora-Everything-iso-${local.fedora44_version}-x86_64-CHECKSUM"
    http_content = {
      "/fedora.ks" = templatefile("${path.root}/fedora.ks", { path = path, hostname = source.name })
    }
  }
